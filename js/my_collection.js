    ...
    success: function (rs) {
      ...
      if (_SG.isReplyOk(rs)) {
        ...
        if (res.length == 0) {
          ...
          // </div>
          // <span class="mr-auto unit_price align-self-baseline">${e.unitPrice}/次</span></div>
          $.each(res, (i, e) => {
            html += `<div class="doll rounded-3" data-id="${e.commodityId}" data-expressPrice="${e.expressPrice}" data-datas='${JSON.stringify(e)}'>
                      ...
                      <div class="doll_info d-flex content-even">
                        <span class="doll_name">${e.commodityName}</span>
...
$('body').on('click','.send-btn', function () {
  ...
  for (let i = 0; i<ele.length; i++) {
    if ($('input[name="choose"]').eq(i).is(':checked')) {
      let data = $('.doll').eq(i).data('datas')
      sendData.push(data)
    }
  }
  sessionStorage.setItem('sendData',JSON.stringify(sendData))
